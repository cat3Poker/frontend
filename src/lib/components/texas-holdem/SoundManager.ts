...
export class SoundManager {
  ...
  private loadSounds() {
    for (const {id, path} of SoundConstants.getSoundDefinitions()) {
  ...
  play(soundId: string, delay: number = 0, volume: number = 1, loop: boolean = false) {
    ...
    if (sound) {
      sound.volume(volume);
      sound.loop(loop)
      setTimeout(() => sound.play(), delay * 1000); // Delay is in seconds
    } else {
      console.warn(`Sound not found: ${soundId}`);
    }
  }

  playSound(soundId: string, delay: number = 0, volume: number = 1, loop: boolean = false) {
    this.play(soundId, delay, volume, loop); // Using the same logic as 'play'
  }
}
...
export class SoundConstants {
  ...
  static getSoundDefinitions() {
    return [
      ...
      { id: this.BUTTON_ROLLOVER, path: "sounds/poker/game/Card_Shuffle.mp3" },
